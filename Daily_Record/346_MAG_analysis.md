...
#then all together
percent <- ggplot(percent_mag_full, aes(y = fraction_class, x = percent, fill = fraction_class, label = scales::percent(percent)))+
  geom_col(width = -.98)+
  labs(x = "Percentage of MAGs")+
  ...
  geom_text(color = "white",
            ...
            fontface = "bold",
            hjust = 1.4)+
  scale_fill_manual(values = c("#9FB6DA","#DF3383", "#759C44")) +
  theme_classic()
percent
```
...
#levels to order our things
congenee_all$fraction_class <- factor(congenee_all$fraction_class, levels = c("Particle", "Generalist", "Free"))

log2fold_mags <- ggplot(mag_full_df, aes(y = log2FoldChange, x = Phylum, fill = Phylum, shape = fraction_class)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5, position = position_dodge(width = 0.7), show.legend = FALSE) +
  geom_point(aes(fill = Phylum), alpha = 0.75, position = position_dodge2(width = 0.4), show.legend = FALSE) +
  scale_fill_manual(values = c(
  "Acidobacteriota"  = "dodgerblue4",
  "Actinobacteriota" = "#C0B956", 
  "Armatimonadota" = "#e46009",
  "Bacteroidota" = "#CE8C1D",
  "Bdellovibrionota" = "#965126",
  "Bdellovibrionota_C" = "#553D32",
  "Cyanobacteria" = "#04d2d5",
  "Firmicutes" = "#80405e",
  "Gemmatimonadota" = "#b2e902",
  "Myxococcota_A" = "#72c998",
  "Planctomycetota" = "#9FB6D9",
  "Proteobacteria" = "#DF3383",
  "Verrucomicrobiota" = "#B994E5"
  )) +
  scale_shape_manual(values = c("Free" = 24, "Generalist" = 21, "Particle" = 25)) +
  guides(fill = guide_legend(override.aes = list(size = 3.3))) +
  xlab("Phylum")+
  ylab("log2FoldChange")+
  theme_classic2() +
...
conj_genes_per_bin <- ggplot(congenee_all, aes(x = fraction_class, y = conj_genes_per_bin, fill = fraction_class, shape = fraction_class))+
  geom_boxplot(outlier.shape = NA, alpha = 0.5, position = position_dodge(width = 0.7)) +
  geom_point(aes(fill = fraction_class), alpha = 0.75, position = position_dodge(width = 0.4), show.legend = TRUE)+
  xlab("Lifestyle")+
...
```r
#now lets save it
ggsave("conjugation_boxplot.png", plot = conj_genes_per_bin, width = 5, height = 4, dpi = 1000)
...
mag_full <- mag_full %>% 
  select(-c("Domain.x", "Phylum.x", "Class.x", "Order.x", "Family.x", "Genus.x", "Species.x", "CUBHE", "GC", "GCdiv", "ConsistencyHE", "CUB", "CPB", "nHE", "dCUB", "d", "LowerCI", "UpperCI", "FilteredSequences", "padj", "growth_class",))
...
new_total <- select(testing_join, -c(fraction_class.x,padj,fraction_class.y,Domain,Phylum,Class,Order,Family,Genus,Species,estimated_length.x))
...
```r
#specify levels
#conjugation gene type level
congenee_type$conj_gene_type <- factor(congenee_type$conj_gene_type,
                                       levels = c("vir genes", "tra genes", "trb genes", "MOB genes"))
#fraction class
congenee_type$fraction_class <- factor(congenee_type$fraction_class, levels = c("Particle", "Generalist", "Free"))
...
gg_conj <- p1 +
  geom_fruit(
    data = congenee_type,
    geom=geom_tile,
    mapping=aes(y=Seq_Name, x=conj_gene_type, fill=conj_gene_type),
    width = 0.35
  ) +
  scale_fill_manual(
    values = c("#313167", "#aa5c3a", "olivedrab1", "#eba2b7"),
...
<img src="figures/ggtreeExtra conjugation genes heatmap-2.png" style="display: block; margin: auto;" />
...
test_merge_phage <- full_join(phage_presence, phab_genes, by=c("Seq_Name"="Seq_Name"))
#now turning the NAs into zeros
test_merge_phage$genes_per_bin_phage[is.na(test_merge_phage$genes_per_bin_phage)] <- 0
#cleaning up our merged file
phab_genes1 <- phab_genes %>% 
  select(-c("Vir_Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species", "CUBHE", "GC", "GCdiv", "ConsistencyHE", "CUB", "CPB", "nHE", "dCUB", "d", "LowerCI", "UpperCI", "FilteredSequences", "growth_class", "fraction_class.x", "Bin", "log2FoldChange","padj", "label", "angle", "Completeness", "fraction_class", "growth_class", "bin_length", "length_mb")) 
```

```
## Error in `select()`:
## ! Can't select columns that don't exist.
## ✖ Column `Vir_Domain` doesn't exist.
```
...
phage_tree <- p2 +
  ...
  scale_fill_manual(
    values = c("#C3774D", "#8B9322", "#932252")
  ) +
  geom_treescale(fontsize=2.9, linesize = .2, x=2)


phage_tree #this is showing us the number of phage encoded genes that are divided by MAG
```
...
#now we will create a dataset from the predicted genes (I have blasted these against blastp and am only going to select it for the sample if it is >80% query and >70% identity other papers have used similar metrics)
```
...
df_plasmidgenes_clean_up <- merge_plasmidgenes_magfull %>% 
...
df_plasmidgenes_clean_up <- df_plasmidgenes_clean_up %>% 
...
# now lets read in our fargene + genomad.csv file. It was easier for me to make the dataset by hand on Excel which is kinda annoying but its fine. It includes information about the seq_name, bin, gene, class of antibiotic, resistance mechanism of ARG merged with the geNomad results
ARG_genomad_fargene_processed <- read.csv("/local/workdir/sna49/SA_Fall2023_Phage_Rotation/Data/fargene/amr_gene_moreprocessed.csv")

#filter out nas
ARG_genomad_fargene_processed_no_NA <- ARG_genomad_fargene_processed[!is.na(ARG_genomad_fargene_processed$ARG_clean), ]


#count args 
count_all_args <- ARG_genomad_fargene_processed_no_NA %>%
  group_by(Seq_Name, fraction_class, estimated_length, log2FoldChange, Phylum, antibiotic_name) %>%
  summarise(count_args = n())

#now lets count how many genes there are per mbp per MAG
count_all_args$count_args <- count_all_args$count_args / count_all_args$estimated_length

#plotting with a bar graph
args_all <- ggplot(count_all_args, aes(x = fraction_class, y = count_args, fill = antibiotic_name))+
  geom_bar(position = "dodge", stat = "identity") +
  xlab("Bacterial Lifestyle")+
  ylab("Number of Antibiotic Resistance Genes per Mbp per MAG")+
  scale_fill_wa_d(palette = "rainier", reverse = TRUE) +
  labs(fill = "args") +
  theme_classic()
args_all
...
```r
#now lets use ggsave to save this plot at a higher dpi and size
ggsave("genomad-fargene-all-ARGs-clean-up.png", plot = args_geNo, width = 5, height = 4, dpi = 1000)
```

```
## Error in eval(expr, envir, enclos): object 'args_geNo' not found
```

```r
#now lets plot it as a box plot. we will do this by combining anitbiotic reistance genes by fraction. we can facet it by each antibiotic type. we will only show the most different ones. 


#box plot

#so here we are looting at the x axis of antibiotic type, y is counting resistance genes and we are filling in the color by fraction class.
#there are a couple of cases like 

#whole box plot view of the x axis as antibiotic name, y is countng genes, and is filled in by fraction class. good to see an overall view. 
all_args <- ggplot(count_all_args, aes(x = antibiotic_name, y = count_args, fill = fraction_class, shape = fraction_class))+
  geom_boxplot(position = position_dodge2(preserve = "single"))+
  geom_point(aes(group = fraction_class), 
             alpha = 0.5, 
...
<img src="figures/geNomad-ARGs-clean-up-2.png" style="display: block; margin: auto;" />

```r
#now we are looking at the x axis of bacterial phylum, y is counting number of ARGs per mbp per MAG 
box_all_args <- ggplot(count_all_args, aes(x = Phylum, y = count_args, fill = fraction_class, shape = fraction_class))+
  geom_boxplot(position = position_dodge2(preserve = "single"))+
  geom_point(aes(group = fraction_class), 
             alpha = 0.5, 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
...
<img src="figures/geNomad-ARGs-clean-up-3.png" style="display: block; margin: auto;" />

```r
#now lets look at everything by fraction class as x axis

#first lets level everything
count_all_args$fraction_class <- factor(count_all_args$fraction_class, levels = c("Particle", "Generalist", "Free"))

#now we are looking at the x axis of fraction class, y is counting number of ARGs per mbp per MAG 
box_all_args_fc <- ggplot(count_all_args, aes(x = fraction_class, y = count_args, fill = fraction_class, shape = fraction_class))+
  geom_boxplot(outlier.shape = NA, alpha = 0.5, position = position_dodge2(preserve = "single"))+
  geom_point(aes(group = fraction_class), 
             alpha = 0.75, 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
             show.legend = TRUE) +
  ...
  stat_compare_means(comparisons = list(
      ...
      c("Free", "Generalist"),
      c("Particle", "Generalist")),
                       method = "wilcox.test", paired = FALSE, label = "p.signif",hide.ns = TRUE,
                       p.adjust.method = "fdr") +
  scale_shape_manual(values = c("Free" = 24, "Generalist" = 21, "Particle" = 25)) +
  xlab("Lifestyle")+
  ylab("Number of ARGs per Mbp per MAG")+
  theme_classic()
box_all_args_fc
```

<img src="figures/geNomad-ARGs-clean-up-4.png" style="display: block; margin: auto;" />

```r
#now lets save it
ggsave("ARGs_boxplot.png", plot = box_all_args_fc, width = 5, height = 4, dpi = 1000)

#now we are looking at the x axis of bacterial phylum, y is counting number of ARGs per mbp per MAG. now this is the faceted version of faceting by fraction class 
facet_arg_fc <- ggplot(count_all_args, aes(x = Phylum, y = count_args, fill = fraction_class, shape = fraction_class))+
  geom_boxplot(position = position_dodge2(preserve = "single"))+
  geom_point(aes(group = fraction_class), 
             alpha = 0.5, 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
             show.legend = TRUE) +
  scale_shape_manual(values = c("Free" = 24, "Generalist" = 21, "Particle" = 25)) +
  facet_wrap(~antibiotic_name, scales = "free") 
facet_arg_fc
```

<img src="figures/geNomad-ARGs-clean-up-5.png" style="display: block; margin: auto;" />

```r
#this is faceted by antibiotic name, x axis is bacterial phylum. y is counting genes
facet_antibiotic <- ggplot(count_all_args, aes(x = Phylum, y = count_args, fill = antibiotic_name, shape = fraction_class))+
  geom_boxplot(position = position_dodge2(preserve = "single"))+
  geom_point(aes(group = fraction_class), 
             alpha = 0.5, 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
...
<img src="figures/geNomad-ARGs-clean-up-6.png" style="display: block; margin: auto;" />

```r
#so when I look at both box plot views the faceted one makes it more inuitive for me to see

#it depends how we want to visualize everything but I like it when we look at bacterial phylum on x axis and having it faceted by antibiotic name. 
  #at thsi view point the ones that are not so interesting are bleomycin, macrolides, viomycin

  #for the poster I would really like to have beta lactams, aminoglycosides, and tetracycline and maybe vancomycine?

#the maybes are chloramphenicol, fluoroquinolone, fosphomycine. 
  # yes fluoroquinoone has all 3 fractoin classes represented so does tetracycline and I find that to be more interesting because I am biased 

#so now to visualize what are in my opinion the most interesting antibiotics (aminoglycosides, beta lactam, tetracycline) i will need to make a filtered dataset to only include these 3
main_three_ab <- c("Aminoglycoside", "Beta Lactam", "Tetracycline")
filtered_args_mainthree <- count_all_args[count_all_args$antibiotic_name %in% main_three_ab, ]

# now lets see her
facet_antibiotic <- ggplot(filtered_args_mainthree, aes(x = Phylum, y = count_args, fill = antibiotic_name, shape = fraction_class)) +
  geom_boxplot(position = position_dodge2(preserve = "single")) +
  geom_point(aes(group = fraction_class), 
             alpha = 0.5, 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
...
<img src="figures/geNomad-ARGs-clean-up-7.png" style="display: block; margin: auto;" />

```r
### or we can make each phylum its own color and we can see
```
...
```r
#now we want to see the log2fold changes according to the mags. we will do this by plotting the log2fold change and coloring them by the bacterial phyla. we will visualize it in a box plot manner

#now we can establish bacterial phylum colors
...
#turns out our phylum column is not a factor so we need to convert it inot one so taht we can plot
mag_full_df$Phylum <- factor(mag_full_df$Phylum)
