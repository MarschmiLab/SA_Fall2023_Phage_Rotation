...
df_meta_assembly_plasmid_sum <- df_meta_assembly_plasmid_sum %>% 
  group_by(Seq_Name, length_mb, fraction) %>%
  summarise(count_n_genes = sum(n_genes))

#now we will divide the number of genes per Mb to get the abundance per assembly
df_meta_assembly_plasmid_sum$genes_per_Mb <- df_meta_assembly_plasmid_sum$length_mb / df_meta_assembly_plasmid_sum$count_n_genes 

#now lets visualize the abundance of plasmid encoded genes per contig 
assembly_plasmid_genes <- ggplot(df_meta_assembly_plasmid_sum, aes(x = fraction, y = genes_per_Mb, fill = fraction)) +
  geom_boxplot() +
  geom_point() +
   geom_pwc(
...
df_meta_assembly_virus_sum <- df_meta_assembly_virus_sum %>% 
  group_by(Sample_ID, length_mb, fraction) %>%
  summarise(count_n_genes = sum(n_genes))
